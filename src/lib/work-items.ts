...
export function buildWorkItems(data: Dataset): WorkItem[] {
  return [
    ...(data.service_occurrences ?? []).map((occurrence) => {
      ...
      const type = data.service_types?.find((item) => item.id === occurrence.service_type_id || item.id === service?.service_type_id);
